...
document.addEventListener('DOMContentLoaded', async function() { // Ajout du mot-clé async ici pour permettre l'utilisation de await dans la fonction
    ...
    // Utilisation d'une boucle for...of pour gérer les promesses retournées par getCoordinatesForCity
    for (let item of concertItems) {
        var text = item.innerText;
        var parts = text.split(' - ');
        var date = parts[0].replace('Date: ', '');
        var city = parts[1].replace('City: ', '');
        ...
        try {
            // Attente de la résolution de la promesse pour obtenir les coordonnées
            var coords = await getCoordinatesForCity(city);
            if (coords) {
                L.marker([coords.lat, coords.lon]).addTo(map)
                    .bindPopup('Date: ' + date + '<br>City: ' + city);
            }
        } catch (error) {
            console.error("Erreur lors de la récupération des coordonnées pour la ville:", city, error);
        }
    }

    async function getCoordinatesForCity(cityCountry) {
        const query = cityCountry.replace("-", " ");
        try {
            ...
            if (data.length === 0) {
                console.error('Aucune donnée trouvée pour cette ville:', cityCountry); // Log supplémentaire pour le débogage
